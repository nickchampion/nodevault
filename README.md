...
5. **Ask your vault** questions in a streamed, cited conversation — a hand-rolled RAG pipeline, fully managed grounding via Vertex AI Search / OpenAI file search, or (with your own OpenRouter key) the same local retrieval answered by any model on **OpenRouter**
...
- **Q&A with citations** — streamed over Server-Sent Events, with three selectable modes per account:
  ...
  - *OpenRouter*: the exact *Local* retrieval stack, but the final answer is generated by any model on [OpenRouter](https://openrouter.ai) that the user picks per conversation. Because OpenRouter has no embeddings, it's a pure *generation override* layered on top of the account's base provider (which still owns embeddings, ingestion and retrieval) rather than a peer provider — so it never touches the pgvector index or the one-way provider switch. The model catalogue is fetched once and cached, searchable, and sorted free-first; the chosen model is persisted with the conversation, and upstream provider errors (rate limits, etc.) are surfaced verbatim so the user can switch models and retry.
...
| AI providers | Google Gemini (`@google/genai`) and OpenAI (`openai`), selectable per account, behind a shared provider interface; optional **OpenRouter** (`openai` SDK against its base URL) as a per-conversation generation override |
...
integrations/
  ...
  openrouter/    OpenRouter client (model catalogue + streamed generation)
